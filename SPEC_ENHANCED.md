...
- **Incremental annotations**: Added `apply_incremental_annotations` helper so out-of-line annotation files can update resolved schemas post-compilation, aligning with the spec's guidance for schema/document separation.
- **Optional static groups**: Added tests confirming nullable static groups encode presence byte `0x01/0xC0` per spec and raise `DecodeError` on invalid markers.
